...
| `allowed_models` | Body | array | 否 | 允许访问的模型列表，空表示不限制。 |
| `allowed_user_channels` | Body | array | 否 | 允许访问的用户渠道 ID 列表，空表示不限制。 |
| `ip_whitelist` | Body | array/string | 否 | IP 白名单。 |
| `quota` | Body | string/number | 否 | API Key 额度。 |
